...
def compare_counts_test(count_PGR1, count_PGR2, print_on=False):
    ...
    if not print_on:
        ...
        print(f"\n{test_type} Results:")
        print(f"T-statistic = {t_stat:.4f}, p-value = {p_value:.4f}")
        print(f"Mean of count_PGR1: {mean1:.6f}")  # Using more precision in display
...
#    plot_frequency_pgr()
    ...
    count_PGR1, count_PGR2 = filtered_data['count_PGR1'].tolist(), filtered_data['count_PGR2'].tolist()
    compare_counts_test(count_PGR1, count_PGR2, print_on=True)
