...
for(i in 3:TotTime) {
    N[i,1] = floor(updateN(rdep(sp1,int1,N[i-1,1],sum(N[i-1,])),z[i,1],N[i-1,1],sum(N[i-1,]))) # ERROR>Lucas: lines 13--18 were commented, so sp_n and int_n were not defined. But after uncommenting there is another error in line 34.
