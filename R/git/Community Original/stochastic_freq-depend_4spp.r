#Stochastic neg-freq depend logistic growth model, 4 species    
...
rdep=function(slope,int,N,Ctot){
    if(Ctot==N) r=1 else r=exp(slope*(N/Ctot-int)) # ERROR>Lucas: the condition has length > 1.
    return(r)
}
...
N = matrix(data=0,nrow=TotTime,ncol=totspp)
N[1:2,1] = initialNsp1
N[1:2,2] = initialNsp2
N[1:2,3] = initialNsp3
N[1:2,4] = initialNsp4
...
for(i in 3:TotTime) {
    N[i,1] = floor(updateN(rdep(sp1,int1,N[i-2,1],sum(N[i-2,])),z[i,1],N[i-1,1],sum(N[i-1,])))
    N[i,2] = floor(updateN(rdep(sp2,int2,N[i-2,2],sum(N[i-2,])),z[i,2],N[i-1,2],sum(N[i-1,])))
    N[i,3] = floor(updateN(rdep(sp3,int3,N[i-2,3],sum(N[i-2,])),z[i,3],N[i-1,3],sum(N[i-1,])))
    N[i,4] = floor(updateN(rdep(sp4,int4,N[i-2,4],sum(N[i-2,])),z[i,4],N[i-1,4],sum(N[i-1,])))
}
