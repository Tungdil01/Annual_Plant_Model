...
# #### - kept the wrong SoS calculation
# #### - kept the parameters' variations from the code
# #### - added an additional filter S1 >= 1 & S2 >= 1, without it I cannot reproduce
#
...
if __name__ == "__main__":
    ...
    results = np.empty((len(mesh), 17), dtype=float)  # Prepare an array to hold the results
...
def analyze_coexistence_effect(file_path, description):
    ...
    print("\nCoexistence and Exclusion based on nu:\n", table_df)
...
def main():
  ...
  # Visualize correlation between factors and coexistence
  cor_figure() # apply filters

  # Analyze coexistence effect in original dataset
  print("1. Analyze the Effect on Coexistence in the Original Dataset by Yenni et al.")
  print("Employs logistic regression to explore how various ecological factors influence the likelihood of species coexistence.")
  analyze_coexistence_effect("csv/annplant_2spp_det_rare.txt", "Original Results by Yenni et al.")

  # Analyze coexistence effect in reproduced dataset
  print("\n\n2. Analyze the Effect on Coexistence Based on Our Reproduced Dataset")
  print("By reproducing the analysis on our generated data, we can validate the original findings and ensure the robustness of our code.")
  analyze_coexistence_effect(output_file, "Reproduction of the Authors' Results")
