...
def generate_parameter_sets(n_samples=2000, eps=1e-3, seed=1234):
    ...
    scaled_samples[:, 0] = samples[:, 0] * (20 - (1 + eps)) + (1 + eps) # r1
    scaled_samples[:, 1] = samples[:, 1] * (20 - (1 + eps)) + (1 + eps) # r2
    scaled_samples[:, 2] = samples[:, 2] * (3 - eps) + eps # a11
    scaled_samples[:, 3] = samples[:, 3] * (3 - eps) + eps # a22
    scaled_samples[:, 4] = samples[:, 4] * (3 - eps) + eps # a12
...
def process_parameter_set(params, n_simulations_per_set=200, max_time=500, seed_offset=0):
    ...
    try:
        ...
        nu_C = calculate_nu(N1_eq, N2_eq, CE1, CE2)
        ...
        median_coexistence_time = np.median(extinction_times)
        ...
        result = {
            ...
            'nu': nu, 'nu_C': nu_C, 'rare_species': rare_status,
            ...
            'median_coexistence_time': median_coexistence_time
        }
        return result
...
def analyze_simulation_results(results_df, metric='nu'):
    ...
    weak_rates = weak_limitation['median_coexistence_time'].values
    strong_rates = strong_limitation['median_coexistence_time'].values
...
def save_results(results_df, filename="demographic_stochasticity_results.csv"):
    ...
    numeric_columns = ['r1', 'r2', 'a11', 'a22', 'a12', 'a21', 'N1_eq', 'N2_eq',
                      ...
                      'mean_coexistence_time', 'median_coexistence_time'
                      ]
...
def find_example_parameter_sets(metrics, results_df, n_additional_samples=50000, seed=1234):
    ...
    for metric in metrics:
        examples[metric] = {
            'negative': {'coexistence': None, 'competitive_exclusion': None},
            'positive': {'coexistence': None, 'competitive_exclusion': None}
        }
    print(f"\nSearching for examples in existing results for metrics: {metrics}...")
    found_counts = {metric: 0 for metric in metrics}
    used_parameter_sets = set()
    for _, row in results_df.iterrows():
        if all(found_counts[metric] >= 4 for metric in metrics):
            break
        ...
        if outcome == 'coexistence':
            if N1_eq <= 1 or N2_eq <= 1:
                continue
        param_id = tuple(round(row[col], 4) for col in ['r1', 'r2', 'a11', 'a22', 'a12', 'a21'])
        for metric in metrics:
            if found_counts[metric] >= 4:
                continue
            value = row[metric]
            sign = classify_sign(value)
            if examples[metric][sign][outcome] is None and param_id not in used_parameter_sets:
                examples[metric][sign][outcome] = row
                found_counts[metric] += 1
                used_parameter_sets.add(param_id)
    ...
    if total_found < total_needed:
        print(f"\nOnly found {total_found}/{total_needed} examples. Generating additional parameter sets...")
        ...
        for params in additional_params:
            ...
            try:
                ...
                row_data = {
                    'r1': r1, 'r2': r2, 'a11': a11, 'a22': a22,
                    'a12': a12, 'a21': a21, 'N1_eq': N1_eq, 'N2_eq': N2_eq,
                    'nu': metric_values['nu'], 'nu_C': metric_values['nu_C']
                }
                param_id = tuple(round(v, 4) for v in [r1, r2, a11, a22, a12, a21])
                for metric in metrics:
                    if found_counts[metric] >= 4:
                        continue
    ...
    for row_idx, sign in enumerate(['negative', 'positive']):
        for metric in metrics:
            ...
            if row is not None:
                ...
                print(f"  r1={params[0]:.3g}, r2={params[1]:.3g}")
                print(f"  a11={params[2]:.3g}, a22={params[3]:.3g}")
